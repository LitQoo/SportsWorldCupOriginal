...
extern "C" {
	...
	void Java_com_litqoo_lib_KSActivityBase_sendWebViewData(JNIEnv *env, jobject thiz, jstring datas, int delekey) // 자바에서 데이터 받음!!
	{
	...
	void Java_com_litqoo_lib_KSActivityBase_sendInAppData(JNIEnv *env, jobject thiz, jstring datas, int resultcode, int delekey) // 자바에서 데이터 받음!!
	{
		jboolean isCopy = JNI_FALSE;
		const char* revStr = env->GetStringUTFChars(datas, &isCopy); // UTF-8 문자를 얻어옴.
		string throwData = revStr;
	...
	void Java_com_litqoo_lib_KSActivityBase_sendDataFromURL(JNIEnv *env, jobject thiz, jstring datas, int delekey) // 자바에서 데이터 받음!!
	{
		jboolean isCopy = JNI_FALSE;
		const char* revStr = env->GetStringUTFChars(datas, &isCopy); // UTF-8 문자를 얻어옴.
